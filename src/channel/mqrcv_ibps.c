		...
		else if(tmpbuf[0]=='@'&&!strcmp(tmpbuf+1,chnlname))
		{
			while(fgets(tmpbuf,sizeof(tmpbuf),fp)!=NULL)
			{
				...
				if(!memcmp(tmpbuf,"交易名称",8))
				{
					...
					if(trancode[0]=='B')
					{
						SysLog(LOG_CHNL_ERR,"静态获取交易代码[%s]\n",trancode+1);
...
int chnlprocess(char *buffer)
{
	...
	SysLog(LOG_CHNL_ERR,"FILE [%s] LINE [%d]:放置交易报文信息到共享内存hash表中成功,跟踪号：[%ld],报文长度[%d]\n",__FILE__,__LINE__,mbuf->innerid,strlen(rcvbuf));
	...
	SysLog(LOG_CHNL_ERR,"FILE [%s] LINE [%d]:获取可用服务并发送控制信号\n",__FILE__,__LINE__);
	/** 发送信号到核心服务 **/
	SysLog(LOG_CHNL_ERR,"FILE [%s] LINE [%d]:准备发送到的服务进程为 [%ld]\n",__FILE__,__LINE__,ipid);
	...
	while (1)
	{
		...
		if (CompCode != MQCC_FAILED)
		{
			...
			}else
			{
				...
				SysLog(LOG_CHNL_ERR,"传入hash表参数[%s]\n",sendbuf);
				if(chnlprocess(sendbuf)==0)
				{
					SysLog(LOG_CHNL_ERR,"渠道处理成功\n");
				}else
				{
