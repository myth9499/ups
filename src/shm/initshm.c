...
int main(int argc,char *argv[])
{
	...
	{
		SysLog(1,"ftok error[%s]",strerror(errno));
		return -1;
	}
	...
	SysLog(1,"get tranmap shm ok \n");

	/** init sem **/
	if(initservregsem()!=0)
	{
		SysLog(1,"init sem id error\n");
		return -1;
	}else
	{
		SysLog(1,"init posix sem ok\n");
	}
	/** system v sem 
	if(init_sem(1)==0)
	{
