...
int send_to_channel(char *chnlname)
{
	...
	{
		SysLog(LOG_SYS_ERR,"获取渠道[%s]消息队列失败\n",chnlname);
		free(mbuf);
		return -1;
	}
	...
	if(iret == -1)
	{
		SysLog(LOG_SYS_ERR,"发送到渠道[%s]失败\n",chnlname);
		free(mbuf);
		return -1;
	}
	free(mbuf);
	SysLog(LOG_SYS_ERR,"发送到渠道[%s]成功\n",chnlname);
	return 0;
}
int recv_from_channel(char *chnlname)
{
	...
	if(iret == -1)
	{
		SysLog(LOG_SYS_ERR,"接收渠道[%s]返回报文失败:%s\n",chnlname,strerror(errno));
		free(mbuf);
		return -1;
	}
	...
	if(tranbuf == NULL)
	{
		SysLog(LOG_SYS_ERR,"申请交易数据内存失败[%s]\n",strerror(errno));
		return -1;
	}
	memset(mbuf->tranbuf.chnlname,0,sizeof(mbuf->tranbuf.chnlname));
